...
class Register {
	...
	function create_account()
	{
		...
		if (is_array($name_check) && isset($name_check['id']))
		{
		...
		if (! $ibforums->vars['allow_dup_email'] )
		{
			...
			if (is_array($email_check) && isset($email_check['id']))
			{
		...
		if($ibforums->vars['reg_chars'])
		{
			for($pos = 0; $pos < strlen($in_username); $pos++)
			{
				if((($in_username[$pos] <= 'я') and ($in_username[$pos] >= 'а')) or (($in_username[$pos] <= 'Я') and ($in_username[$pos] >= 'А')) or (($in_username[$pos] <= 'z') and ($in_username[$pos] >= 'a')) or (($in_username[$pos] <= 'Z') and ($in_username[$pos] >= 'A')) or (($in_username[$pos] <= '9') and ($in_username[$pos] >= '0')))
				{
					//do nothing
				}
				else
				{
					$std->Error( array( 'LEVEL' => 1, 'MSG' => 'invalid_chars' ) );
					return;
				}
			}
		}
		...
		if ($coppa != 1)
		{
			if ( ($ibforums->vars['reg_auth_type'] == 'user') or ($ibforums->vars['reg_auth_type'] == 'admin') ) {
				...
				if ( $ibforums->vars['reg_auth_type'] == 'user' )
				{
					...
					$this->email->subject = "Регистрация на форуме ".$ibforums->vars['board_name'];
					$this->email->to      = $member['email'];
				...
				if ($ibforums->vars['new_reg_notify']) {
					...
					$this->email->subject = "Новый зарегистрированный на ".$ibforums->vars['board_name'];
