...
if ( ($ibforums->input['c'] ?? "") != "" )       
{
		...
		if ($ibforums->vars['show_birthdays'])
		{
			...
			else
			{
				...
				if ( ! ($ibforums->vars['autohide_bday'] ?? 0) )
				{
					$stats_html .= $this->html->birthdays( $birthstring, $count, $lang  );
				}
			}
		...
		if ( $ibforums->vars['show_calendar'] ?? 0 )
		{
		...
		if ($ibforums->member['id']) {
			...
			$row = $DB->fetch_row();
			$data['tt_id']   = $row['id']   ?? 0;
$data['tt_name'] = $row['name'] ?? 'None';
$data['tt_num']  = $row['num']  ?? 0;

$DB->query("SELECT `id`,`name`,`posts` FROM `ibf_members` WHERE 1 AND id > 0 ORDER BY `posts` DESC LIMIT 1");
$row = $DB->fetch_row();

$data['tp_id']   = $row['id']    ?? 0;
$data['tp_name'] = $row['name']  ?? 'Guest';
$data['tp_num']  = $row['posts'] ?? 0;
        ...
        foreach ($this->cats as $cat_id => $cat_data)
        {
            ...
            foreach ($this->forums as $forum_id => $forum_data)
            {
                if ($forum_data['category'] == $cat_id)
                {
                    ...
                    $temp_html = ($temp_html ?? "") . $this->process_forum($forum_id ?? 0, $forum_data ?? []);
                }
            }
