...
class post_functions extends Post {
	...
	function process($class) {
		...
		if ($ibforums->input['QPost'])
		{
			...
			$this->quoted_post['DATE'] = $std->get_date_old( $ibforums->input['QDate'], 'LONG' );
																	   
			if ( $class->parser->error != "" )
			{
				$class->obj['post_errors'] = $class->parser->error;
				
				$class->dump_quote = "<div class='quote-wrapper'><b>" . $this->quoted_post['AUTHOR']['name'] . "</b> (" . $this->quoted_post['DATE'] . ")<div class='quote-content'>" . trim($ibforums->input['QPost']) . "</div></div><p>&nbsp;</p>\n";
			...
			if ( $class->obj['preview_post'] == "" )
			{
				...
				$this->post['post'] = "<div class='quote-wrapper'><b>" . $this->quoted_post['AUTHOR']['name'] . "</b> (" . $this->quoted_post['DATE'] . ")<div class='quote-content'>" 
                    . trim($ibforums->input['QPost']) 
                    . "</div></div><p>&nbsp;</p>\n\n" 
                    . $this->post['post'];
			}
			else
...
function add_reply($class) {
	...
	function show_form($class) {
		...
		if (isset($raw_post))
		{
			...
			if ( isset($class->dump_quote) && $class->dump_quote != "" )
			{
...
{
    // This looks for your modern div wrapper and wipes it out to prevent nesting
    $this->quoted_post['post'] = preg_replace( "#<div class='quote-wrapper'>.+?</div></div>#is", "", $this->quoted_post['post'] );
}
