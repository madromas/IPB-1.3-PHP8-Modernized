...
class post_functions extends Post {
	...
	function __construct($class) {
		...
		if ( ! is_array($this->orig_post) or ! $this->orig_post['pid'] )
		{
			$std->Error( array( 'LEVEL' => 1, 'MSG' => 'missing_files') );
		}
		...
		if ($this->orig_post['attach_id'])
		{
			if ( isset($ibforums->input['editupload']) AND ($ibforums->input['editupload'] != 'keep') )
			{
				...
				else if ($ibforums->input['editupload'] == 'delete')
				{
					...
					$this->post['attach_id']   = intval($this->post['attach_id']);
$this->post['attach_hits'] = intval($this->post['attach_hits']);
$this->post['author_id']   = intval($this->post['author_id']);
$this->post['icon_id']     = intval($this->post['icon_id']);
				}
			}
			else
			{
					...
					if ( ($this->moderator['edit_topic'] == 1) OR ( $ibforums->member['g_is_supmod'] == 1 ) )
{
    ...
    $safe_tid = intval($this->topic['tid']);
    $safe_pid = intval($this->post['pid']);
    $safe_fid = intval($class->forum['id']);
    // ---------------------------------
...
$this->post['author_id']   = intval($this->post['author_id']);
$this->post['icon_id']     = intval($this->post['icon_id']);
$this->post['topic_id']    = intval($this->post['topic_id']);
$this->post['queued']      = intval($this->post['queued']);
	...
	function show_form($class) {
		...
		$raw_post = isset($_POST['Post'])  ? $std->txt_htmlspecialchars($_POST['Post']) : $class->parser->unconvert($this->orig_post['post'], $class->forum['use_ibc'], $class->forum['use_html']);
