...
class Topics {
    ...
    function __construct()
    {
        ...
        if (is_array($this->topic) && isset($this->topic['forum_id']))
{
    $this->forum = array( 
...
{
    // Fallback to prevent the rest of the script from crashing
    $this->forum    = array();
    $this->category = array();
}
...
//-------------------------------------
// Update the topic views counter (Unique Fix)
//-------------------------------------

if ( ! strstr( $std->my_getcookie('topics_read'), ",".$this->topic['tid']."," ) )
{
...
if ( is_array($this->moderator) AND isset($this->moderator['delete_post']) AND $this->moderator['delete_post'] == 1 ) return $button;

if ( isset($poster['id']) AND isset($ibforums->member['id']) AND $poster['id'] == $ibforums->member['id'] )
{
    if ( isset($ibforums->member['g_delete_own_posts']) AND $ibforums->member['g_delete_own_posts'] == 1 )
    {
        return $button;
    }
}
...
if ( is_array($this->moderator) AND isset($this->moderator['edit_post']) AND $this->moderator['edit_post'] == 1 ) return $button;
	...
	function view_ip($row, $poster)
	{
		...
		if ( ($ibforums->member['g_is_supmod'] != 1) && (!is_array($this->moderator) || $this->moderator['view_ip'] != 1) )
		{
			return "";
		}
		else
	...
	function multi_moderation()
	{
		...
		if ( $ibforums->member['id'] )
		{
			if ( isset($ibforums->member['g_is_supmod']) AND $ibforums->member['g_is_supmod'] == 1 )
{
    $pass_go = TRUE;
}
else if ( is_array($this->moderator) AND isset($this->moderator['can_mm']) AND $this->moderator['can_mm'] == 1 )
{
    $pass_go = TRUE;
}
		}
	...
	function append_link( $key="" ) {
		...
		// start oska pin first hack modified
		if (($this->topic['firstpost'] ?? '0') == '1' and $key == 'PIN_FIRST') return "";
if (($this->topic['firstpost'] ?? '0') == '0' and $key == 'UNPIN_FIRST') return "";
		// end oska pin first hack modified
