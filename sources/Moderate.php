...
class Moderate {
	...
	function split_complete() {
		...
		$this->moderate_log("Splitting the topic '{$this->topic['title']}'");
	...
	function merge_complete() {
		...
		$this->moderate_log("Merging topic '{$old_topic['title']}' with '{$this->topic['title']}'");
	...
	function poll_edit_do() {
		...
		$this->moderate_log("Editing poll in topic");
	...
	function do_move() {
		...
		$this->moderate_log("Moving topic from $source_name to $dest_name");
	...
	function delete_post() {
		...
		$this->moderate_log("Deleting a message");
	...
	function delete_topic()
	{
		...
		$this->moderate_log("Deleting a topic");
	...
	function do_edit()
	{
		...
		$this->moderate_log("Editing topic title: (ID: {$this->topic['tid']}) from '{$this->topic['title']}' to '$topic_title'");
	...
	function open_topic()
	{
		...
		$this->moderate_log("Opening a closed topic");
	...
	function close_topic()
	{
		...
		$this->moderate_log("Closing a topic");
	...
	/*************************************************/
	// PIN FIRST POST IN TOPIC:
	// ---------------
	//
	/*************************************************/
	
	function pin_first_post() {
		global $std, $ibforums, $DB, $print;
    ...
    if ($this->moderator['pin_first_post_topic'] == 1) {
		...
		$this->moderate_log("Pinning the first post of the topic");
		...
		@header("Location: ?act=ST&f=".$this->forum['id']."&t=".$this->topic['tid'] );
		
	}

	// start oska modified pin first hack
	/*************************************************/
	// UNPIN FIRST POST IN TOPIC:
	// ---------------
	//
	/*************************************************/
	
	function unpin_first_post() {
		global $std, $ibforums, $DB, $print;
		...
		if ( (isset($this->moderator['pin_first_post_topic']) && $this->moderator['pin_first_post_topic'] == 1) OR $ibforums->member['g_is_supmod'] == 1 )
{
    $passed = 1;
		...
		$this->moderate_log("Unpinning the first fixed post of the topic");
	...
	function pin_topic()
	{
		...
		$this->moderate_log("Pinning the topic");
	...
	function unpin_topic()
	{
		...
		$this->moderate_log("Unpinning the topic");
