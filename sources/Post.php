...
// 1. CLEANED TAG STRING (Removed regex-style pipes/parents for Purifier compatibility)
$allowed_tags = 'div[class|style],b,strong,i[class|style],em,a[href|title|target],ul,ol,li,p[style|class],br,span[style|class],img[width|height|alt|src],iframe[src|width|height|frameborder|allowfullscreen|allow|sandbox],video[src|controls|width|height|poster|preload],source[src|type],blockquote,pre[class],code[class]';
...
$def->addAttribute('pre', 'data-mce-highlighted', 'Bool');
$def->addAttribute('pre', 'data-mce-selected', 'Text');
$def->addAttribute('pre', 'contenteditable', 'Enum#false');

$def->addAttribute('iframe', 'allow', 'Text');
$def->addAttribute('iframe', 'sandbox', 'Text');
$def->addAttribute('iframe', 'allowfullscreen', 'Bool');

$def->addElement('video', 'Block', 'Optional: source', 'Common', [
    'src'      => 'URI',
    'type'     => 'Text',
    'width'    => 'Length',
    'height'   => 'Length',
...
class Post {
	...
	{
		global $ibforums, $std, $REQUEST_METHOD, $_POST, $purifier;

// Set a massive fallback for post length if not defined
$ibforums->vars['max_post_length'] = $ibforums->vars['max_post_length'] ?: 2140000;

// PHP 8 Safe Moderator Check
if ( (is_array($this->moderator) && isset($this->moderator['mid']) && $this->moderator['mid'] != "" && $ibforums->member['id'] != 0) 
     || (isset($ibforums->member['g_is_supmod']) && $ibforums->member['g_is_supmod'] == 1) ) 
{
    $modflag = TRUE;
} 
else 
...
$raw_post = $_POST['Post'] ?? ''; 
$purified_post = $purifier->purify($raw_post);

// 2. Now build the array using the already purified variable
$post = array(
    ...
    'post'        => $purified_post, // No legacy parser call here
	...
	function process_upload() {
		...
		global $ibforums, $std, $_FILES, $DB, $FILE_UPLOAD;
...
$FILE_SIZE = $_FILES['FILE_UPLOAD']['size'] ?? 0;
$FILE_TYPE = $_FILES['FILE_UPLOAD']['type'] ?? '';
		...
		if ( ($_FILES['FILE_UPLOAD']['name'] ?? "") == "" || ($_FILES['FILE_UPLOAD']['name'] ?? "") == "none" )
{
    return $attach_data;
}
		//-------------------------------------------------
	...
	function mod_options($is_reply=0) {
		...
		{
			if ( is_array($this->moderator) AND isset($this->moderator['mid']) AND $this->moderator['mid'] != "" )
			{
	...
	function moderate_log($topic_title, $title = 'unknown') {
    global $std, $ibforums, $DB, $QUERY_STRING;
    
    $safe_fid = intval($ibforums->input['f']);
    $safe_tid = intval($ibforums->input['t']);
    $safe_pid = intval($ibforums->input['p']);
    // ---------------------------------
