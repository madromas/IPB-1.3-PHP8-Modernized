...
//----PORTAL---------------
			...
			case 'warn':
				...
				break;
			case 'dowarn':
				$_POST['warn_protected'] = ','. @implode( ",", (array)($_POST['groups'] ?? []) ) . ',';
    ...
    function portal()
{
    ...
    $top_polls[] = array(0, "Do not display polls");
    $top_polls[] = array(0, "-----------------------");
    ...
    $form_array[] = array (0, "None, I will use the settings below");

    // Header Translation: 'Portal', 'Configure portal settings carefully'
    $this->common_header('doportal', 'Portal', 'Please configure the portal settings carefully');

    $ADMIN->html .= $SKIN->add_td_row( array( "<b>Portal Poll</b><br>Select a poll to display (only topics with polls are shown here)." ,
                                              $SKIN->form_dropdown( "portal_poll", $top_polls , $INFO['portal_poll'] )
                                     )      );

    $ADMIN->html .= $SKIN->add_td_row( array( "<b>From which forum to export news?</b>" ,
                                              $SKIN->form_dropdown( "portal_newsforum", $form_array, $INFO['portal_newsforum']  )
                                     )      );

    $ADMIN->html .= $SKIN->add_td_row( array( "<b>From which forums to export news (Expert)?</b><br>Enter Forum IDs separated by commas (,)" ,
                                              $SKIN->form_input( "portal_newsforum_expert", $INFO['portal_newsforum_expert']  )
                                     )      );
    
    $ADMIN->html .= $SKIN->add_td_row( array( "<b>How many news posts to display on the home page?</b>" ,
                                              $SKIN->form_input( "portal_newsposts", $INFO['portal_newsposts']  )
                                     )      );

    $ADMIN->html .= $SKIN->add_td_row( array( "<b>Show Google Search bar?</b>" ,
                                              $SKIN->form_yes_no( "portal_googlebar", $INFO['portal_googlebar']  )
                                     )      );

    $ADMIN->html .= $SKIN->add_td_row( array( "<b>Show Latest Topics (Simple Version)?</b>" ,
                                              $SKIN->form_yes_no( "portal_latestposts", $INFO['portal_latestposts']  )
                                     )      );

    $ADMIN->html .= $SKIN->add_td_row( array( "<b>Number of Latest Topics to display (Simple)?</b>" ,
                                              $SKIN->form_input( "portal_num_latestposts", $INFO['portal_num_latestposts']  )
                                     )      );

    $ADMIN->html .= $SKIN->add_td_row( array( "<b>Show Latest Topics (Expanded Version)?</b>" ,
                                              $SKIN->form_yes_no( "portal_latestposts_big", $INFO['portal_latestposts_big']  )
                                     )      );

    $ADMIN->html .= $SKIN->add_td_row( array( "<b>Number of Latest Topics to display (Expanded)?</b>" ,
                                              $SKIN->form_input( "portal_num_latestposts_big", $INFO['portal_num_latestposts_big']  )
                                     )      );

    $ADMIN->html .= $SKIN->add_td_row( array( "<b>Show Forum Navigation?</b>" ,
    ...
    $ADMIN->html .= $SKIN->add_td_row( array( "<b>Show Board Statistics (Total posts, members, etc.)?</b>" ,
    ...
    $ADMIN->html .= $SKIN->add_td_row( array( "<b>Show Top Posters?</b>" ,
                                              $SKIN->form_yes_no( "portal_top_posters", $INFO['portal_top_posters']  )
                                     )      );

    $ADMIN->html .= $SKIN->add_td_row( array( "<b>Number of Top Posters to display?</b>" ,
                                              $SKIN->form_input( "portal_num_topposters", $INFO['portal_num_topposters']  )
                                     )      );

    $ADMIN->html .= $SKIN->add_td_row( array( "<b>Show Latest Registered Members?</b>" ,
                                              $SKIN->form_yes_no( "portal_new_members", $INFO['portal_new_members']  )
                                     )      );

    $ADMIN->html .= $SKIN->add_td_row( array( "<b>Number of New Members to display?</b>" ,
                                              $SKIN->form_input( "portal_num_newmembers", $INFO['portal_num_newmembers']  )
                                     )      );

    $ADMIN->html .= $SKIN->add_td_row( array( "<b>Show Most Active Forums?</b>" ,
                                              $SKIN->form_yes_no( "portal_top_forums", $INFO['portal_top_forums']  )
                                     )      );

    $ADMIN->html .= $SKIN->add_td_row( array( "<b>Number of Active Forums to display?</b>" ,
                                              $SKIN->form_input( "portal_num_top_forums", $INFO['portal_num_top_forums']  )
                                     )      );
                                     
    $ADMIN->html .= $SKIN->add_td_row( array( "<b>Show Login Box (for guests)?</b>" ,
                                              $SKIN->form_yes_no( "portal_loginbox", $INFO['portal_loginbox']  )
                                     )      );

    $ADMIN->html .= $SKIN->add_td_row( array( "<b>Show 'Posts since last visit' to users?</b><br>Not recommended if you have Latest Topics enabled, as they are very similar." ,
                                              $SKIN->form_yes_no( "portal_newposts", $INFO['portal_newposts']  )
                                     )      );

    $ADMIN->html .= $SKIN->add_td_row( array( "<b>Number of 'Posts since last visit' to display?</b>" ,
                                              $SKIN->form_input( "portal_num_newposts", $INFO['portal_num_newposts']  )
                                     )      );

    $ADMIN->html .= $SKIN->add_td_row( array( "<b>Show Old News?</b>" ,
                                              $SKIN->form_yes_no( "portal_old_news", $INFO['portal_old_news']  )
                                     )      );

    $ADMIN->html .= $SKIN->add_td_row( array( "<b>Number of Old News items to display:</b>" ,
                                              $SKIN->form_input( "portal_num_old_news", $INFO['portal_num_old_news']  )
                                     )      );
                                     
    $ADMIN->html .= $SKIN->add_td_row( array( "<b>Show Welcome Panel?</b>" ,
                                              $SKIN->form_yes_no( "portal_welcomepanel", $INFO['portal_welcomepanel']  )
                                     )      );

    $ADMIN->html .= $SKIN->add_td_row( array( "<b>Show Random Member block?</b>" ,
                                              $SKIN->form_yes_no( "portal_member_moment", $INFO['portal_member_moment']  )
                                     )      );
                                     
    $ADMIN->html .= $SKIN->add_td_row( array( "<b>Trim news text after X characters?</b>" ,
                                              $SKIN->form_yes_no( "portal_tease_news", $INFO['portal_tease_news']  )
                                     )      );

    $ADMIN->html .= $SKIN->add_td_row( array( "<b>Character limit for news [Read More] link?</b><br>Note: Large limits may cause issues with HTML/BBCode tags in the text." ,
                                              $SKIN->form_input( "portal_tease_length", $INFO['portal_tease_length']  )
                                     )      );

    $ADMIN->html .= $SKIN->add_td_row( array( "<b>Show Calendar Events?</b>" ,
	...
	{
		global $IN, $INFO, $DB, $SKIN, $ADMIN, $std, $MEMBER, $GROUP, $_FILES;
...
$ADMIN->html .= $SKIN->add_td_row( array( 
    "<b>HTML Detection Regex</b><br />The regex pattern used to detect modern HTML (TinyMCE) content." ,
    $SKIN->form_input( "html_detection_regex", $INFO['html_detection_regex'] )
) );

$ADMIN->html .= $SKIN->add_td_row( array( 
    "<b>HTMLPurifier Allowed Tags/Attributes</b><br />Tags and [attributes] permitted in posts." ,
    $SKIN->form_textarea( "html_purifier_allowed", $INFO['html_purifier_allowed'] ?: 'div[class|style],b,strong,i[class|style],em,a[href|title],ul,ol,li,p[style|class],br,span[style|class],img[width|height|alt|src]' )
) );
