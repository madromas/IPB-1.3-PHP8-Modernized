...
class admin_functions {
	...
	function build_tree() {
		...
		// Ensure 'show' key exists to prevent regex warning
		$in_show = $IN['show'] ?? "";
		...
		foreach($CATS as $cid => $name)
		{
			...
			if ( preg_match( "/(?:^|,)$cid(?:,|$)/", $in_show ) )
			{
				// Check if the category actually has pages defined
				if ( isset($PAGES[ $cid ]) && is_array($PAGES[ $cid ]) )
				{
				...
				$links = ""; // Reset links instead of unset to avoid notice in next loop
			}
			else
			{
