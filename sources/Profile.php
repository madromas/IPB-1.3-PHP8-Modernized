...
class Profile {
    ...
    if ( ($this->photo_member['photo_type'] ?? null) and ($this->photo_member['photo_location'] ?? null) )
    	{
    		$this->has_photo = TRUE;
...
while($row = $DB->fetch_row()) {
    ...
    if($INFO['latest_show']) {
        $info['last_five'] .= "<tr><td class='row1'>{$row['post']}</td></tr>";
    }
}
    	//---------------------------------------------------
    	// Output
...
{
    // Execute the function first to get the HTML links
    $edit_links = $this->html->user_edit($info);
    
    // Replace the comment tag with the actual HTML result
    $this->output = str_replace( "<!--MEM OPTIONS-->", $edit_links, $this->output );
}
    	...
    	if ( $ibforums->vars['warn_on'] and ( ! stristr( $ibforums->vars['warn_protected'], ','.$member['mgroup'].',' ) ) )
		{
			if ($ibforums->member['id'])
			{
				...
				else
				{
					...
					if ( is_array($this->moderator) AND ($this->moderator['mid'] AND $this->moderator['allow_warn'] == 1) )
					{
						$pass = 1;
						$mod  = 1;
					}
