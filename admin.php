...
else
{
	$out_val = $IN['out'] ?? '';
$show_val = $IN['show'] ?? '';
$IN['show'] = preg_replace( "/(?:^|,)" . preg_quote($out_val, '/') . "(?:,|$)/", ",", $show_val );
...
$DB->query("UPDATE ibf_admin_sessions SET RUNNING_TIME='".time()."', LOCATION='".($IN['act'] ?? '')."' WHERE MEMBER_ID='".($MEMBER['id'] ?? 0)."' AND ID='".($IN['AD_SESS'] ?? '')."'");
